...
def get_hex_from_bgr(bgr_color: List[int]) -> str:
    """
    Converts a BGR color list (from OpenCV) to a web-friendly HEX string.
    """
...
def hex_to_lab(hex_color: str) -> List[float]:
    ...
    try:
        ...
        logging.error(f"Error converting HEX {hex_color} to LAB: {e}")
        return [0.0, 0.0, 0.0] # Fallback on failure